#ifndef GLFRAMEBUFFERWORK_H
#define GLFRAMEBUFFERWORK_H

#include "GLWork.h"
class GLFrameBufferWork :public GLWork
{
public:
	GLFrameBufferWork(const GLchar* vertexPath, const GLchar* fragmentPath, glm::vec3 cameraPos);
	~GLFrameBufferWork();
...
GLFrameBufferWork::GLFrameBufferWork(const GLchar* vertexPath, const GLchar* fragmentPath, glm::vec3 cameraPos) :GLWork(vertexPath, fragmentPath, cameraPos)
{
}

GLFrameBufferWork::~GLFrameBufferWork()
{
}

void GLFrameBufferWork::onDraw(int workIndex)
{
	if (workIndex == firstDraw)
	{

		Shader drawFboProgram = Shader("glsl//vertex_FrameBuffer.vs", "glsl//fragment_FrameBuffer.fra");
		...
		glEnableVertexAttribArray(drawFboProgram.getAttributeLocation("position"));
		glVertexAttribPointer(drawFboProgram.getAttributeLocation("position"), 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), (void*)0);
		glEnableVertexAttribArray(drawFboProgram.getAttributeLocation("texCoords"));
		glVertexAttribPointer(drawFboProgram.getAttributeLocation("texCoords"), 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), (void*)(2 * sizeof(GLfloat)));
		glBindVertexArray(0);
		...
		GLuint textureForBuffer;
		glGenTextures(1, &textureForBuffer);
		glBindTexture(GL_TEXTURE_2D, textureForBuffer);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, WIDTH, HEIGHT, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glBindTexture(GL_TEXTURE_2D, 0);

		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, textureForBuffer, 0);
	...
	{
		Shader drawFboProgram = Shader("glsl//vertex_FrameBuffer2.vs", "glsl//fragment_FrameBuffer2.fra");
		// Setup some OpenGL options
		glEnable(GL_DEPTH_TEST);
		glDepthFunc(GL_LESS); // Set to always pass the depth test (same effect as glDisable(GL_DEPTH_TEST))
